"""
Griffin architecture: Real-Gated Linear Recurrent Unit (RG-LRU) blocks interleaved
with local sliding-window attention blocks, following arxiv.org/abs/2402.19427.

Block pattern: [RNN, RNN, LocalAttn] x (num_layers // 3)
MLP: GeGLU gated MLP with expansion 3 (replaces relu²)
"""
...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 9))  # should be divisible by 3
    model_dim = int(os.environ.get("MODEL_DIM", 512))
    num_heads = int(os.environ.get("NUM_HEADS", 8))
    num_kv_heads = int(os.environ.get("NUM_KV_HEADS", 1))
    mlp_expansion = int(os.environ.get("MLP_EXPANSION", 3))  # GeGLU expansion
    rnn_width = int(os.environ.get("RNN_WIDTH", 0))  # 0 = auto: 4*model_dim//3
    local_attn_window = int(os.environ.get("LOCAL_ATTN_WINDOW", 512))
    rglru_c = float(os.environ.get("RGLRU_C", 8.0))
    ...
    ttt_eval_loops = int(os.environ.get("TTT_EVAL_LOOPS", 1))
...
class GatedMLP(nn.Module):
    """GeGLU gated MLP: two parallel projections, one gated by GELU."""
...
class RGLRU(nn.Module):
    """
    Real-Gated Linear Recurrent Unit (Section 2.4 of Griffin paper).

    Recurrence: h_t = a_t * h_{t-1} + sqrt(1 - a_t^2) * (i_t * x_t)
    where:
        r_t = sigmoid(W_r x_t)               # recurrence gate
        i_t = sigmoid(W_i x_t)               # input gate
        log(a_t) = -c * softplus(lam) * r_t  # stable form of a_t = sigmoid(lam)^(c*r_t)
    """
    def __init__(self, dim: int, c: float = 8.0):
        super().__init__()
        self.c = c
        self.W_r = CastedLinear(dim, dim, bias=True)
        self.W_i = CastedLinear(dim, dim, bias=True)
        # lam: log-scale recurrence parameter; init so sigmoid(lam)^c ~ Uniform(0.9, 0.999)
        self.lam = nn.Parameter(torch.empty(dim, dtype=torch.float32))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        log_a = -self.c * F.softplus(self.lam.to(dtype=x.dtype)) * r  # (B, T, D)
        a = log_a.exp()
        b = (1.0 - a.square()).clamp(min=0.0).sqrt() * (i * x)
...
class RecurrentMixingBlock(nn.Module):
    ...
    Input x (B, T, model_dim) → two parallel branches → merge → output (B, T, model_dim)
      Branch 1 (temporal): Linear → depthwise Conv1d(kernel=4) → RG-LRU
      Branch 2 (gate):     Linear → GELU
      Merge: element-wise product → output Linear
    """
    def __init__(self, model_dim: int, rnn_width: int, c: float = 8.0):
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor:
        B, T, _ = x.shape
        z = self.in_proj_z(x)   # (B, T, rnn_width)
        g = self.in_proj_g(x)   # (B, T, rnn_width)

        # Causal depthwise conv: pad left by kernel_size-1
        z_t = z.transpose(1, 2)  # (B, rnn_width, T)
        z_t = F.pad(z_t, (3, 0))  # kernel_size - 1 = 3
        z_t = self.conv(z_t)      # (B, rnn_width, T)
        z = z_t.transpose(1, 2)   # (B, T, rnn_width)
...
class LocalSlidingAttention(nn.Module):
    """Multi-head local (sliding window) attention."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class GriffinBlock(nn.Module):
    """
    One Griffin residual block: pre-norm temporal-mixing + pre-norm MLP.
    The temporal-mixing layer is either RecurrentMixingBlock or LocalSlidingAttention.
    """
    def __init__(self, temporal_layer: nn.Module, dim: int, mlp_expansion: int):
        super().__init__()
        self.temporal_norm = RMSNorm()
...
class GriffinModel(nn.Module):
    """
    Griffin: blocks follow pattern [RNN, RNN, LocalAttn] x (num_layers // 3).
    U-net style skip connections same as GPT in train_simple.py.
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        super().__init__()
        if num_layers % 3 != 0:
            raise ValueError(f"num_layers must be divisible by 3 for Griffin pattern, got {num_layers}")
        ...
        for i in range(num_layers):
            if i % 3 == 2:
                ...
                temporal = LocalSlidingAttention(
                    model_dim, num_heads, num_kv_heads, local_attn_window, rope_base, qk_gain_init
                )
            else:
                # Recurrent block
                temporal = RecurrentMixingBlock(model_dim, rnn_width, c=rglru_c)
            blocks.append(GriffinBlock(temporal, model_dim, mlp_expansion))
...
# Only attaches to LocalSlidingAttention blocks (index % 3 == 2).
# -----------------------------
...
class BatchedGriffinTTTLoRA(nn.Module):
    """LoRA adapters for Griffin: LM head + Q/V for each local-attention block."""

    def __init__(self, bsz: int, model: GriffinModel, rank: int):
        super().__init__()
        dim = model.tok_emb.embedding_dim
        vocab = model.tok_emb.num_embeddings
        self.lm_head_lora = BatchedLinearLoRA(bsz, dim, vocab, rank)
        # Per-block loras; None for recurrent blocks, BatchedLinearLoRA for attn blocks
        self.q_loras: list[BatchedLinearLoRA | None] = []
        self.v_loras: list[BatchedLinearLoRA | None] = []
        self._q_mods = nn.ModuleList()
        self._v_mods = nn.ModuleList()
        for block in model.blocks:
            if isinstance(block.temporal, LocalSlidingAttention):
                attn = block.temporal
                ql = BatchedLinearLoRA(bsz, dim, attn.c_q.weight.shape[0], rank)
                vl = BatchedLinearLoRA(bsz, dim, attn.c_v.weight.shape[0], rank)
                self.q_loras.append(ql)
                self.v_loras.append(vl)
                self._q_mods.append(ql)
                self._v_mods.append(vl)
            else:
...
def _build_ttt_optimizer(lora, args: Hyperparameters):
    return torch.optim.Adam(
        lora.parameters(), lr=args.ttt_lora_lr, betas=(args.beta1, args.beta2), eps=1e-10
    )
...
def _find_docs(all_tokens: Tensor, include_next_bos: bool = True) -> list[tuple[int, int]]:
    ...
    for i in range(len(bos_positions)):
        start = int(bos_positions[i])
            ...
            int(bos_positions[i + 1]) if i + 1 < len(bos_positions) else all_tokens.numel()
        )
...
def eval_val_ttt_lora(
    ...
    lora = BatchedGriffinTTTLoRA(batch_size, base_model, lora_rank).to(device)
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        if bsz == batch_size:
            ...
            cur_lora = BatchedGriffinTTTLoRA(bsz, base_model, lora_rank).to(device)
...
def main() -> None:
    ...
    rnn_width = args.rnn_width if args.rnn_width > 0 else 4 * args.model_dim // 3
    ...
    log0(f"griffin:rnn_width:{rnn_width} local_attn_window:{args.local_attn_window} rglru_c:{args.rglru_c}")
    log0(f"griffin:num_layers:{args.num_layers} (pattern=[RNN,RNN,LocalAttn]x{args.num_layers//3})")
    log0(
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        if step < args.lr_warmup_steps:
            return (step + 1) / max(args.lr_warmup_steps, 1)
    ...
    while True:
        ...
        if should_validate:
            ...
            log_wandb(wandb_run, {"val/loss": val_loss, "val/bpb": val_bpb,
                                   "train/time_ms": training_time_ms,
                                   "train/step_avg_ms": training_time_ms / max(step, 1)}, step=step)
            torch.cuda.synchronize()
            t0 = time.perf_counter()
