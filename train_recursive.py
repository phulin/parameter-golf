...
class Hyperparameters:
    ...
    ttt_eval_loops = int(os.environ.get("TTT_EVAL_LOOPS", 1))  # separate: TTT has many more fwd passes per doc
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, num_loops=num_loops).detach()
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None, num_loops: int = 1) -> Tensor:
        ...
        for ci in range(max_nc):
            ...
            if needs_train:
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ptl = base_model(x, y, lora=cur_lora, num_loops=num_loops)
            else:
...
def main() -> None:
    ...
    resolved_eval_loops = args.eval_loops if args.eval_loops > 0 else args.max_loops
    log0(f"recursive_loops:max_loops:{args.max_loops} eval_loops:{resolved_eval_loops}")
    wandb_run = None
    if master_process:
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, num_loops=step_loops)
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, num_loops=step_loops)
