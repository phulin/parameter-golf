...
def main() -> None:
    ...
    lr_warmup_ms = 10_000.0

    def lr_mul(step: int, elapsed_ms: float) -> float:
        if elapsed_ms < lr_warmup_ms:
            return elapsed_ms / lr_warmup_ms
        if max_wallclock_ms is not None:
            progress = min((elapsed_ms - lr_warmup_ms) / max(max_wallclock_ms - lr_warmup_ms, 1.0), 1.0)
        else:
